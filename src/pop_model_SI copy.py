...
def run_sim(sim_count):
    ...
    #----------------
    method = 'random'
    dir = "data/SI/full distribution/sim{}/{}/".format(sim_count, method)
    ...
    n_steps = 50 # MC steps
    step = 0
    for N in range(1, 10000, int(10000./50)):
        step += 1
        # create new state i.e. new set of
        # random / normal distributed locations
        L = 20 * sqrt(10)
        cells = np.random.normal(size=(N, 2)) * L / 3
        n = N * 0.0001
        ...
        for ni in save_densities:
            if abs(n - ni) < stol:
                fState = dir + "state_N_%d.csv" % (N)
                np.savetxt(fState, cells, fmt="%d")
